...
function buildClinicalFocusRule(subject) {
  ...
  if (isBasic) return "";
  return `Каждое задание — это короткая клиническая ситуация с конкретным пациентом (возраст, пол, 1-3 ключевых симптома или данных) и одним чётким врачебным вопросом. Вопрос проверяет способность принять клиническое решение: поставить диагноз, выбрать тактику, объяснить механизм, предотвратить осложнение.

Примеры:
«У ребёнка 3 лет температура 39°C, ригидность затылочных мышц, петехиальная сыпь. Ваш предварительный диагноз?»
«Новорождённый на 2-е сутки: нарастающая желтуха, вялость, гемоглобин 100 г/л. Какова наиболее вероятная причина?»
«Ребёнок 8 лет с бронхиальной астмой получает сальбутамол, эффект снижается. Какова Ваша тактика ведения?»
«У девочки 12 лет после ангины появились боли в суставах и кольцевидная эритема. Какое осложнение наиболее вероятно?»`;
}
...
Ð¤Ð¾Ñ€Ð¼Ð°Ñ‚: Ñ‚Ð¾Ð»ÑŒÐºÐ¾ Ñ‚ÐµÐºÑÑ‚ Ð·Ð°Ð´Ð°Ð½Ð¸Ñ Ð±ÐµÐ· Ð½ÑƒÐ¼ÐµÑ€Ð°Ñ†Ð¸Ð¸, Ð·Ð°Ð³Ð¾Ð»Ð¾Ð²ÐºÐ¾Ð² Ð¸ Ð¾Ñ‚Ð²ÐµÑ‚Ð¾Ð².`;
  ...
  const clinicalCheckCritic = buildClinicalFocusRule(context.subject)
    ? `
10. ${buildClinicalFocusRule(context.subject)}`
    : "";
